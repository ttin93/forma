...
export default function OnboardingPage() {
  ...
  const [brand, setBrand] = useState({ name: '', website: '', color: '#0a0a0a' });
    ...
    return (
      <header style={{ height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 32px', borderBottom: '1px solid var(--color-line)', background: '#fff' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, fontSize: 12, color: 'var(--color-text-3)', fontFamily: 'var(--font-mono)' }}>
          ...
          <button style={{ all: 'unset', color: 'var(--color-text-2)', cursor: 'pointer', fontSize: 12 }} onClick={() => router.push('/dashboard')}>
            Skip setup
          </button>
        </div>
      </header>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
                {industries.map(c => (
                  <button key={c.id} onClick={() => setIndustry(c.id)} style={{
                    all: 'unset', padding: 18, border: industry === c.id ? '1.5px solid #0a0a0a' : '1px solid var(--color-line-2)',
                    borderRadius: 'var(--radius-3)', cursor: 'pointer', background: industry === c.id ? '#fafafa' : '#fff',
                    height: 110, display: 'flex', flexDirection: 'column', justifyContent: 'space-between',
                  }}>
                    <div style={{ width: 24, height: 24, border: '1px solid var(--color-line)', borderRadius: 'var(--radius-2)' }} />
                    <div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  <div>
                    ...
                    <div style={{ border: '1px dashed var(--color-line-3)', borderRadius: 'var(--radius-2)', padding: 18, display: 'flex', alignItems: 'center', gap: 14 }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, fontWeight: 500 }}>Upload logo</div>
                        <div style={{ fontSize: 12, color: 'var(--color-muted)' }}>SVG, PNG or JPG · max 2 MB</div>
                      </div>
                      <Btn variant="ghost" size="sm">Browse</Btn>
                    </div>
                  </div>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--color-text-2)', marginBottom: 6 }}>Primary brand color</div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', border: '1px solid var(--color-line-2)', borderRadius: 'var(--radius-2)' }}>
                      <div style={{ width: 22, height: 22, borderRadius: 4, background: brand.color }} />
                ...
                <div>
                  ...
                  <div style={{ border: '1px solid var(--color-line)', borderRadius: 'var(--radius-3)', overflow: 'hidden' }}>
                    ...
                    <div style={{ padding: 16, background: '#f9f9f9' }}>
                      <div style={{ fontSize: 12, color: 'var(--color-text-3)', marginBottom: 8 }}>Configure your pergola</div>
